		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDNSV2RecordSetExists("telefornicaopencloud_dns_recordset_v2.recordset_1", &recordset),
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_dns_recordset_v2.recordset_1", "description", "a record set"),
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_dns_recordset_v2.recordset_1", "records.0", "10.1.0.0"),
				),
			},
			resource.TestStep{
				Config: testAccDNSV2RecordSet_update(zoneName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("telefornicaopencloud_dns_recordset_v2.recordset_1", "name", zoneName),
					resource.TestCheckResourceAttr("telefornicaopencloud_dns_recordset_v2.recordset_1", "ttl", "6000"),
					resource.TestCheckResourceAttr("telefornicaopencloud_dns_recordset_v2.recordset_1", "type", "A"),
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_dns_recordset_v2.recordset_1", "description", "an updated record set"),
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_dns_recordset_v2.recordset_1", "records.0", "10.1.0.1"),
				),
			},
		},
	})
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(
						"telefornicaopencloud_dns_recordset_v2.recordset_1", "ttl", regexp.MustCompile("^[0-9]+$")),
				),
			},
		},
	})
...
func testAccCheckDNSV2RecordSetDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_dns_recordset_v2" {
			continue
		}
...
func testAccDNSV2RecordSet_basic(zoneName string) string {
	return fmt.Sprintf(`
		resource "telefornicaopencloud_dns_zone_v2" "zone_1" {
			name = "%s"
		...
		resource "telefornicaopencloud_dns_recordset_v2" "recordset_1" {
			zone_id = "${telefornicaopencloud_dns_zone_v2.zone_1.id}"
			name = "%s"
			type = "A"

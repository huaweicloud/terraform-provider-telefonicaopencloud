			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_networking_router_v2.router_1", "name", "router_2"),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_networking_router_v2.router_1", "external_network_id", OS_EXTGW_ID),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2RouterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_networking_router_v2" {
			continue
		}

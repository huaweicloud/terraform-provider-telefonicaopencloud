		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2PoolExists("telefornicaopencloud_lb_pool_v2.pool_1", &pool),
				),
			},
			resource.TestStep{
				Config: TestAccLBV2PoolConfig_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("telefornicaopencloud_lb_pool_v2.pool_1", "name", "pool_1_updated"),
				),
			},
		},
	})
...
func testAccCheckLBV2PoolDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_lb_pool_v2" {
			continue
		}

			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_objectstorage_container_v1.container_1", "name", "container_1"),
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_objectstorage_container_v1.container_1", "content_type", "application/json"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"telefornicaopencloud_objectstorage_container_v1.container_1", "content_type", "text/plain"),
				),
			},
		},
	})
...
func testAccCheckObjectStorageV1ContainerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_objectstorage_container_v1" {
			continue
		}

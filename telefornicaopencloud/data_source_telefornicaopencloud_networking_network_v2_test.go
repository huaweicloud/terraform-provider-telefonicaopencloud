		...
		Steps: []resource.TestStep{
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingNetworkV2DataSourceID("data.telefornicaopencloud_networking_network_v2.net"),
					resource.TestCheckResourceAttr(
						"data.telefornicaopencloud_networking_network_v2.net", "name", "tf_test_network"),
					resource.TestCheckResourceAttr(
						"data.telefornicaopencloud_networking_network_v2.net", "admin_state_up", "true"),
				),
			},
		},
	})
...
resource "telefornicaopencloud_networking_network_v2" "net" {
        name = "tf_test_network"
        admin_state_up = "true"
}

resource "telefornicaopencloud_networking_subnet_v2" "subnet" {
  name = "tf_test_subnet"
  cidr = "192.168.199.0/24"
  no_gateway = true
  network_id = "${telefornicaopencloud_networking_network_v2.net.id}"
}
`
...
data "telefornicaopencloud_networking_network_v2" "net" {
	name = "${telefornicaopencloud_networking_network_v2.net.name}"
}
...
data "telefornicaopencloud_networking_network_v2" "net" {
	matching_subnet_cidr = "${telefornicaopencloud_networking_subnet_v2.subnet.cidr}"
}
`, testAccOpenStackNetworkingNetworkV2DataSource_network)

		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV1PoolExists("telefornicaopencloud_lb_pool_v1.pool_1", &pool),
					resource.TestCheckResourceAttr("telefornicaopencloud_lb_pool_v1.pool_1", "lb_provider", "haproxy"),
				),
			},
			resource.TestStep{
				Config: testAccLBV1Pool_update,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("telefornicaopencloud_lb_pool_v1.pool_1", "name", "pool_1"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckComputeV2InstanceExists("telefornicaopencloud_compute_instance_v2.instance_1", &instance1),
					testAccCheckComputeV2InstanceExists("telefornicaopencloud_compute_instance_v2.instance_2", &instance2),
					testAccCheckLBV1PoolExists("telefornicaopencloud_lb_pool_v1.pool_1", &pool),
					testAccCheckLBV1MonitorExists("telefornicaopencloud_lb_monitor_v1.monitor_1", &monitor),
					testAccCheckLBV1VIPExists("telefornicaopencloud_lb_vip_v1.vip_1", &vip),
				),
			},
			resource.TestStep{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SubnetExists("telefornicaopencloud_networking_subnet_v2.subnet_1", &subnet),
					testAccCheckLBV1PoolExists("telefornicaopencloud_lb_pool_v1.pool_1", &pool),
					testAccCheckLBV1MonitorExists("telefornicaopencloud_lb_monitor_v1.monitor_1", &monitor_1),
					testAccCheckLBV1MonitorExists("telefornicaopencloud_lb_monitor_v1.monitor_2", &monitor_2),
				),
			},
			resource.TestStep{
...
func testAccCheckLBV1PoolDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_lb_pool_v1" {
			continue
		}
...
resource "telefornicaopencloud_lb_pool_v1" "pool_1" {
  name = "pool_1"
  protocol = "HTTP"
...
resource "telefornicaopencloud_compute_instance_v2" "instance_2" {
  name = "instance_2"
  security_groups = ["default", "${telefornicaopencloud_compute_secgroup_v2.secgroup_1.name}"]
...
resource "telefornicaopencloud_lb_monitor_v1" "monitor_1" {
  type = "TCP"
  delay = 30
  timeout = 5
  max_retries = 3
...
resource "telefornicaopencloud_lb_pool_v1" "pool_1" {
  ...
  protocol = "TCP"
  lb_method = "ROUND_ROBIN"
  monitor_ids = ["${telefornicaopencloud_lb_monitor_v1.monitor_1.id}"]
  subnet_id = "${telefornicaopencloud_networking_subnet_v2.subnet_1.id}"
}

resource "telefornicaopencloud_lb_member_v1" "member_1" {
  port = 80
  admin_state_up = true
  pool_id = "${telefornicaopencloud_lb_pool_v1.pool_1.id}"
  address = "${telefornicaopencloud_compute_instance_v2.instance_1.access_ip_v4}"
}

resource "telefornicaopencloud_lb_member_v1" "member_2" {
  port = 80
  admin_state_up = true
  pool_id = "${telefornicaopencloud_lb_pool_v1.pool_1.id}"
  address = "${telefornicaopencloud_compute_instance_v2.instance_2.access_ip_v4}"
}

resource "telefornicaopencloud_lb_vip_v1" "vip_1" {
  name = "vip_1"
  protocol = "TCP"
  port = 80
  admin_state_up = true
...
resource "telefornicaopencloud_lb_monitor_v1" "monitor_2" {
  type = "TCP"
  delay = 30
  timeout = 5
  max_retries = 3
...
resource "telefornicaopencloud_lb_pool_v1" "pool_1" {
  ...
  protocol = "TCP"
  lb_method = "ROUND_ROBIN"
  monitor_ids = ["${telefornicaopencloud_lb_monitor_v1.monitor_2.id}"]
  subnet_id = "${telefornicaopencloud_networking_subnet_v2.subnet_1.id}"
}
`

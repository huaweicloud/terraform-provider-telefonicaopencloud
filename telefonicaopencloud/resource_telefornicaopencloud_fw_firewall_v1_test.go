		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1("telefornicaopencloud_fw_firewall_v1.fw_1", "", "", policyID),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1(
						"telefornicaopencloud_fw_firewall_v1.fw_1", "fw_1", "terraform acceptance test", policyID),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1Exists("telefornicaopencloud_fw_firewall_v1.fw_1", &firewall),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1Exists("telefornicaopencloud_fw_firewall_v1.fw_1", &firewall),
					resource.TestCheckResourceAttr("telefornicaopencloud_fw_firewall_v1.fw_1", "description", "firewall router test"),
...
func testAccCheckFWFirewallV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_firewall" {
			continue
		}
...
resource "telefornicaopencloud_fw_firewall_v1" "fw_1" {
  policy_id = "${telefornicaopencloud_fw_policy_v1.policy_1.id}"
...
resource "telefornicaopencloud_fw_policy_v1" "policy_1" {
  name = "policy_1"
}
`
...
resource "telefornicaopencloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${telefornicaopencloud_fw_policy_v1.policy_2.id}"
  admin_state_up = true
...
resource "telefornicaopencloud_fw_policy_v1" "policy_2" {
  name = "policy_2"
}
`
...
resource "telefornicaopencloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${telefornicaopencloud_fw_policy_v1.policy_1.id}"
  associated_routers = ["${telefornicaopencloud_networking_router_v2.router_1.id}"]
}
`
...
resource "telefornicaopencloud_networking_router_v2" "router_2" {
  name = "router_2"
  admin_state_up = "true"
  distributed = "false"
}
...
resource "telefornicaopencloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${telefornicaopencloud_fw_policy_v1.policy_1.id}"
  associated_routers = [
    "${telefornicaopencloud_networking_router_v2.router_1.id}",
    "${telefornicaopencloud_networking_router_v2.router_2.id}"
  ]
}
`

		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2FloatingIPExists("telefonicaopencloud_compute_floatingip_v2.fip_1", &fip),
				),
			},
		},
	})
...
func testAccCheckComputeV2FloatingIPDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_compute_floatingip_v2" {
			continue
		}
...
resource "telefonicaopencloud_compute_floatingip_v2" "fip_1" {
}
`

			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2NetworkExists(
						"telefonicaopencloud_networking_network_v2.network_1", &network[0]),
					testAccCheckNetworkingV2SubnetExists(
						"telefonicaopencloud_networking_subnet_v2.subnet_1", &subnet[0]),
					testAccCheckNetworkingV2NetworkExists(
						"telefonicaopencloud_networking_network_v2.network_1", &network[1]),
					testAccCheckNetworkingV2SubnetExists(
						"telefonicaopencloud_networking_subnet_v2.subnet_1", &subnet[1]),
					testAccCheckNetworkingV2RouterInterfaceExists(
						"telefonicaopencloud_networking_router_interface_v2.int_1"),
					testAccCheckNetworkingV2RouterInterfaceExists(
						"telefonicaopencloud_networking_router_interface_v2.int_2"),
					testAccCheckNetworkingV2RouterRouteExists(
						"telefonicaopencloud_networking_router_route_v2.router_route_1"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterRouteExists(
						"telefonicaopencloud_networking_router_route_v2.router_route_1"),
					testAccCheckNetworkingV2RouterRouteExists(
						"telefonicaopencloud_networking_router_route_v2.router_route_2"),
				),
			},
			resource.TestStep{
				Config: testAccNetworkingV2RouterRoute_destroy,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2RouterRouteEmpty("telefonicaopencloud_networking_router_v2.router_1"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2RouterRouteDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_networking_router_route_v2" {
			continue
		}
...
resource "telefonicaopencloud_networking_router_interface_v2" "int_2" {
  router_id = "${telefonicaopencloud_networking_router_v2.router_1.id}"
  port_id = "${telefonicaopencloud_networking_port_v2.port_2.id}"
}
...
resource "telefonicaopencloud_networking_router_route_v2" "router_route_2" {
  destination_cidr = "10.0.2.0/24"
  next_hop = "192.168.200.254"

  depends_on = ["telefonicaopencloud_networking_router_interface_v2.int_2"]
  router_id = "${telefonicaopencloud_networking_router_v2.router_1.id}"
}
`

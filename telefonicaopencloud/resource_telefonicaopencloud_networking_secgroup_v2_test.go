			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2SecGroupExists(
						"telefonicaopencloud_networking_secgroup_v2.secgroup_1", &security_group),
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPtr(
						"telefonicaopencloud_networking_secgroup_v2.secgroup_1", "id", &security_group.ID),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_networking_secgroup_v2.secgroup_1", "name", "security_group_2"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_networking_secgroup_v2" {
			continue
		}

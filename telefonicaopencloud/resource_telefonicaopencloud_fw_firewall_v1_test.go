		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1("telefonicaopencloud_fw_firewall_v1.fw_1", "", "", policyID),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1(
						"telefonicaopencloud_fw_firewall_v1.fw_1", "fw_1", "terraform acceptance test", policyID),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1Exists("telefonicaopencloud_fw_firewall_v1.fw_1", &firewall),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWFirewallV1Exists("telefonicaopencloud_fw_firewall_v1.fw_1", &firewall),
					resource.TestCheckResourceAttr("telefonicaopencloud_fw_firewall_v1.fw_1", "description", "firewall router test"),
...
func testAccCheckFWFirewallV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_firewall" {
			continue
		}
...
resource "telefonicaopencloud_fw_firewall_v1" "fw_1" {
...
resource "telefonicaopencloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${telefonicaopencloud_fw_policy_v1.policy_2.id}"
  admin_state_up = true
...
resource "telefonicaopencloud_fw_policy_v1" "policy_2" {
  name = "policy_2"
}
`
...
resource "telefonicaopencloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${telefonicaopencloud_fw_policy_v1.policy_1.id}"
  associated_routers = ["${telefonicaopencloud_networking_router_v2.router_1.id}"]
}
`
...
resource "telefonicaopencloud_networking_router_v2" "router_2" {
  name = "router_2"
  admin_state_up = "true"
  distributed = "false"
}
...
resource "telefonicaopencloud_fw_firewall_v1" "fw_1" {
  ...
  policy_id = "${telefonicaopencloud_fw_policy_v1.policy_1.id}"
  associated_routers = [
    "${telefonicaopencloud_networking_router_v2.router_1.id}",
    "${telefonicaopencloud_networking_router_v2.router_2.id}"
  ]
}
`

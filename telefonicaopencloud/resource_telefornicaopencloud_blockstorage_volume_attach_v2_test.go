		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBlockStorageVolumeAttachV2Exists("telefornicaopencloud_blockstorage_volume_attach_v2.va_1", &va),
				),
			},
		},
	})
...
func testAccCheckBlockStorageVolumeAttachV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_blockstorage_volume_attach_v2" {
			continue
		}
...
resource "telefornicaopencloud_blockstorage_volume_attach_v2" "va_1" {

...
func testAccCheckNetworkingV2FloatingIPDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_networking_floatingip_v2" {
			continue
		}
...
resource "telefornicaopencloud_networking_router_interface_v2" "router_interface_1" {
...
resource "telefornicaopencloud_networking_port_v2" "port_1" {
  admin_state_up = "true"
  network_id = "${telefornicaopencloud_networking_subnet_v2.subnet_1.network_id}"
...
resource "telefornicaopencloud_networking_floatingip_v2" "fip_1" {
  ...
  fixed_ip = "${telefornicaopencloud_networking_port_v2.port_1.fixed_ip.1.ip_address}"
}

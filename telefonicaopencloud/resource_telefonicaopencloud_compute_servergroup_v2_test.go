		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2ServerGroupExists("telefonicaopencloud_compute_servergroup_v2.sg_1", &sg),
				),
			},
		},
	})
...
func testAccCheckComputeV2ServerGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_compute_servergroup_v2" {
			continue
		}
...
resource "telefonicaopencloud_compute_servergroup_v2" "sg_1" {
  name = "sg_1"
  policies = ["affinity"]
}
`
...
resource "telefonicaopencloud_compute_instance_v2" "instance_1" {
  ...
  scheduler_hints {
    group = "${telefonicaopencloud_compute_servergroup_v2.sg_1.id}"
  }
  network {
    uuid = "%s"
  }

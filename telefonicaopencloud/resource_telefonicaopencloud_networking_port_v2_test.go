		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2PortExists("telefonicaopencloud_networking_port_v2.port_1", &port),
				),
			},
		},
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2SubnetExists("telefonicaopencloud_networking_subnet_v2.vrrp_subnet", &subnet),
					testAccCheckNetworkingV2NetworkExists("telefonicaopencloud_networking_network_v2.vrrp_network", &network),
					testAccCheckNetworkingV2PortExists("telefonicaopencloud_networking_port_v2.vrrp_port_1", &vrrp_port_1),
					testAccCheckNetworkingV2PortExists("telefonicaopencloud_networking_port_v2.vrrp_port_2", &vrrp_port_2),
					testAccCheckNetworkingV2PortExists("telefonicaopencloud_networking_port_v2.instance_port", &instance_port),
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_networking_port_v2.port_1", "all_fixed_ips.0", "192.168.199.23"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_networking_port_v2.port_1", "all_fixed_ips.1", "192.168.199.24"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2PortDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_networking_port_v2" {
			continue
		}
...
resource "telefonicaopencloud_networking_network_v2" "vrrp_network" {
  name = "vrrp_network"
  admin_state_up = "true"
}

resource "telefonicaopencloud_networking_subnet_v2" "vrrp_subnet" {
  name = "vrrp_subnet"
  cidr = "10.0.0.0/24"
  ip_version = 4
  network_id = "${telefonicaopencloud_networking_network_v2.vrrp_network.id}"
...
resource "telefonicaopencloud_networking_router_v2" "vrrp_router" {
  name = "vrrp_router"
}

resource "telefonicaopencloud_networking_router_interface_v2" "vrrp_interface" {
  router_id = "${telefonicaopencloud_networking_router_v2.vrrp_router.id}"
  subnet_id = "${telefonicaopencloud_networking_subnet_v2.vrrp_subnet.id}"
}

resource "telefonicaopencloud_networking_port_v2" "vrrp_port_1" {
  name = "vrrp_port_1"
  admin_state_up = "true"
  network_id = "${telefonicaopencloud_networking_network_v2.vrrp_network.id}"
...
resource "telefonicaopencloud_networking_port_v2" "vrrp_port_2" {
  name = "vrrp_port_2"
  admin_state_up = "true"
  network_id = "${telefonicaopencloud_networking_network_v2.vrrp_network.id}"
...
resource "telefonicaopencloud_networking_port_v2" "instance_port" {
  name = "instance_port"
  admin_state_up = "true"
  network_id = "${telefonicaopencloud_networking_network_v2.vrrp_network.id}"

  allowed_address_pairs {
    ip_address = "${telefonicaopencloud_networking_port_v2.vrrp_port_1.fixed_ip.0.ip_address}"
    mac_address = "${telefonicaopencloud_networking_port_v2.vrrp_port_1.mac_address}"
  }

  allowed_address_pairs {
    ip_address = "${telefonicaopencloud_networking_port_v2.vrrp_port_2.fixed_ip.0.ip_address}"
    mac_address = "${telefonicaopencloud_networking_port_v2.vrrp_port_2.mac_address}"
  }
}
`
...
resource "telefonicaopencloud_networking_port_v2" "instance_port" {
  ...
  admin_state_up = "true"
  network_id = "${telefonicaopencloud_networking_network_v2.vrrp_network.id}"
  security_group_ids = ["${telefonicaopencloud_networking_secgroup_v2.secgroup_1.id}"]
...
resource "telefonicaopencloud_networking_port_v2" "port_1" {
  ...
  network_id = "${telefonicaopencloud_networking_network_v2.network_1.id}"
  security_group_ids = [
    "${telefonicaopencloud_networking_secgroup_v2.secgroup_1.id}",
    "${telefonicaopencloud_networking_secgroup_v2.secgroup_2.id}"
  ]
...
resource "telefonicaopencloud_networking_port_v2" "port_1" {
  ...
  admin_state_up = "true"
  network_id = "${telefonicaopencloud_networking_network_v2.network_1.id}"
  security_group_ids = ["${telefonicaopencloud_networking_secgroup_v2.secgroup_2.id}"]

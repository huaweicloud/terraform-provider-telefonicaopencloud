		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2RouterInterfaceExists("telefonicaopencloud_networking_router_interface_v2.int_1"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2RouterInterfaceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_networking_router_interface_v2" {
			continue
		}

		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2VolumeAttachExists("telefornicaopencloud_compute_volume_attach_v2.va_1", &va),
				),
			},
		},
	})
...
func testAccCheckComputeV2VolumeAttachDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_compute_volume_attach_v2" {
			continue
		}
...
resource "telefornicaopencloud_compute_volume_attach_v2" "va_1" {
  instance_id = "${telefornicaopencloud_compute_instance_v2.instance_1.id}"
  volume_id = "${telefornicaopencloud_blockstorage_volume_v2.volume_1.id}"
}
`

	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2InstanceExists("telefonicaopencloud_compute_instance_v2.instance_1", &instance),
					testAccCheckNetworkingV2FloatingIPExists("telefonicaopencloud_networking_floatingip_v2.fip_1", &fip_1),
					testAccCheckNetworkingV2FloatingIPExists("telefonicaopencloud_networking_floatingip_v2.fip_2", &fip_2),
...
func testAccCheckComputeV2FloatingIPAssociateDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_compute_floatingip_associate_v2" {
			continue
		}
...
resource "telefonicaopencloud_compute_floatingip_associate_v2" "fip_1" {
  floating_ip = "${telefonicaopencloud_networking_floatingip_v2.fip_1.address}"
  instance_id = "${telefonicaopencloud_compute_instance_v2.instance_1.id}"
  fixed_ip = "${telefonicaopencloud_compute_instance_v2.instance_1.access_ip_v4}"
}
`, OS_NETWORK_ID)
...
resource "telefonicaopencloud_compute_floatingip_associate_v2" "fip_1" {
  floating_ip = "${telefonicaopencloud_networking_floatingip_v2.fip_1.address}"
  instance_id = "${telefonicaopencloud_compute_instance_v2.instance_1.id}"
  fixed_ip = "${telefonicaopencloud_compute_instance_v2.instance_1.network.0.fixed_ip_v4}"
}
`, OS_NETWORK_ID)
...
resource "telefonicaopencloud_networking_floatingip_v2" "fip_2" {
}
...
resource "telefonicaopencloud_compute_floatingip_associate_v2" "fip_1" {
  floating_ip = "${telefonicaopencloud_networking_floatingip_v2.fip_2.address}"
  instance_id = "${telefonicaopencloud_compute_instance_v2.instance_1.id}"
}
`, OS_NETWORK_ID)

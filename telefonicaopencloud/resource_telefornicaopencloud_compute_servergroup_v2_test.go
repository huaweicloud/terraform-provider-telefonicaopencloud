		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2ServerGroupExists("telefornicaopencloud_compute_servergroup_v2.sg_1", &sg),
				),
			},
		},
	})
...
func testAccCheckComputeV2ServerGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefornicaopencloud_compute_servergroup_v2" {
			continue
		}
...
resource "telefornicaopencloud_compute_servergroup_v2" "sg_1" {
  name = "sg_1"
  policies = ["affinity"]
}
`
...
resource "telefornicaopencloud_compute_instance_v2" "instance_1" {
  ...
  scheduler_hints {
    group = "${telefornicaopencloud_compute_servergroup_v2.sg_1.id}"
  }
}
`
